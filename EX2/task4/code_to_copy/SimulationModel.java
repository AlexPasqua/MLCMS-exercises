...
public abstract class SimulationModel<T extends DefaultSimulationConfig> extends DefaultModel {
	...
	protected final Color susceptibleColor = new Color(255, 255, 0); // yellow for SUSCEPTIBLE
	protected final Color recoveredColor = new Color(0, 255, 0);     // green  for RECOVERED
	...
	public SimulationModel(final T config) {
		...
		this.colorMap.put(SIRType.ID_INFECTED.ordinal(), getInfectiveColor());
		this.colorMap.put(SIRType.ID_SUSCEPTIBLE.ordinal(), getSusceptibleColor());
		this.colorMap.put(SIRType.ID_RECOVERED.ordinal(), getRecoveredColor());
	...
	// getter methods for various colors
	public Color getSusceptibleColor() {
		return susceptibleColor;
	}

	public Color getInfectiveColor() {
		return infectiveColor;
	}
